...
export default function PublicLayout() {
  ...
  return (
    <div className="font-sans antialiased min-h-screen bg-white">
        ...
        <div className="w-full h-16 px-8 md:px-12 lg:px-24 flex items-center justify-end relative">
          ...
          <Link to="/" onClick={() => setMobileMenuOpen(false)} className="absolute left-8 md:left-12 lg:left-24 top-0 flex items-center gap-2 z-[110]">
            <img 
              ...
              alt="Pain Away" 
              className={`h-[12.5rem] w-auto transition-all duration-300 ${!scrolled && (location.pathname === '/' || location.pathname.startsWith('/products') || location.pathname.startsWith('/services')) ? 'brightness-0 invert drop-shadow-2xl' : 'drop-shadow-none scale-[0.6] -ml-8'}`} 
            />
          </Link>
      ...
      {/* Page Content */}
      <main className={location.pathname === '/' || location.pathname.startsWith('/products') || location.pathname.startsWith('/services') ? '' : 'pt-24 px-8 md:px-12 lg:px-24'}>
        <Outlet />
      </main>
    </div>
  );
